...
func checkAuthStatus() {
        if OAuth2TokenStorage.shared.token != nil {
            print("Токен в хранилище: \(OAuth2TokenStorage.shared.token ?? "nil")")
            
            // Переход на WebViewViewController
            print("[SplashViewController.checkAuthStatus]: Токен найден - переход в приложение")
            let webVC = WebViewViewController()
            UIApplication.shared.windows.first?.rootViewController = webVC
        } else {
            // Переход на AuthViewController (программно)
